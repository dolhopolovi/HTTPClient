...
class CurlBuilderTest extends TestCase
{
    public function testSetGenericCurlExtraParamPack(): void {
        ...
        $genericCurlRequestDTOExpected = (new GenericCurlExtraParamPack())
            ->setCURLOPTVERBOSE(true)
            ->setCURLOPTFAILONERROR(false)
            ->setCURLOPTRETURNTRANSFER(true)
            ->setCURLOPTHEADER(true)->get();
    ...
    public function provideCurlOptCheckData(): array {
        return [
            [(new CurlBuilder())->setGenericCurlExtraParamPack()
                                ->setCURLOPTURL('http://example.com')
                                ->setCURLOPTSTDERR(new FileLoggerService())
                                ->setCURLOPTCUSTOMREQUEST('GET')
            ],
            [(new CurlBuilder())->setGenericCurlExtraParamPack()
                                ->setCURLOPTURL('http://example.com')
                                ->setCURLOPTSTDERR(new FileLoggerService())
                                ->setCURLOPTSSLVERIFYPEER()
            ],
            [(new CurlBuilder())->setCURLOPTURL('http://example.com')
                                ->setCURLOPTSTDERR(new FileLoggerService())
                                ->setCURLOPTCUSTOMREQUEST('GET')
                                ->setCURLOPTSSLVERIFYPEER()
            ],
    ...
    public function testBuildRequest() {
        $requestActual = (new CurlBuilder())
            ->setGenericCurlExtraParamPack()
            ->setCURLOPTURL('http://example.com')
            ->setCURLOPTSTDERR(new FileLoggerService())
            ->setCURLOPTCUSTOMREQUEST('GET')
            ->setCURLOPTSSLVERIFYPEER()
            ->buildRequest()->getPSRRequest();

        $requestExpected = (new RequestBuilder())->setUri('http://example.com')->setMethod(EHttpMethod::GET)->getRequest();

        if($requestActual->getUri()->__toString() === $requestExpected->getUri()->__toString()
           && $requestExpected->getMethod() === $requestActual->getMethod()) {
            $this->assertTrue(true);
        } else
            $this->fail();
    }
